...
def linear(a, b, bias):
    ...
    @triton.autotune(configs=linear_configs, key=["M", "N", "K"])
    @triton.jit
    def linear_kernel(
        a_ptr,
        b_ptr,
    ...
    a_t = a if isinstance(a, torch.Tensor) else torch.as_tensor(a, device="cuda")
    b_t = b if isinstance(b, torch.Tensor) else torch.as_tensor(b, device="cuda")
    bias_t = bias if isinstance(bias, torch.Tensor) else torch.as_tensor(bias, device="cuda")
    if not a_t.is_cuda or not b_t.is_cuda or not bias_t.is_cuda:
        raise ValueError("Triton linear requires CUDA tensors")
    M, K = a_t.shape
    K2, N = b_t.shape
    if K != K2:
        raise ValueError("Incompatible shapes")
